...
class DDPMTrainer():
    ...
    def p_sample(self, x_0: torch.Tensor, t: int) -> torch.Tensor:
        ...
        x_t_1 = (x_t - term_2) / torch.sqrt(self.alphas[:, t, ...].unsqueeze(1))
        return x_t_1 + torch.sqrt(self.betas[:, t, ...].unsqueeze(1)) * torch.randn_like(x_0)
